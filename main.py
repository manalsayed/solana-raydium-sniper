...
class Config_CLI():
    ...
    async def prompt_rpc_url():
        ...
        rpc_url = await inquirer.text(message="Enter your Solana RPC URL endpoint or press ENTER to skip (If unsure, use https://api.mainnet-beta.solana.com): ").execute_async()
    ...
    async def add_token_snipe(self):
        ...
        while True:
            ...
            if confirm == "Yes":
                ...
                # minutes = 30
                timestamp = int((datetime(2025, int(month), int(day), int(hours), int(minutes)).timestamp()))
...
class Wallets_CLI():
    ...
    @staticmethod
    async def update_wallet_balance(wallet_id: str, sol_balance: float):
        """Updates the sol_balance of a wallet in wallets.json."""
        wallets = await Wallets_CLI.get_wallets()
        wallets[wallet_id]['sol_balance'] = sol_balance
    ...
    async def display_wallets():
        ...
        for wallet_id, wallet_data in wallets.items():
            ...
            await Wallets_CLI.update_wallet_balance(wallet_id, sol_balance)
    ...
    async def display_selected_wallet():
        ...
        dataframe = tabulate(pd.DataFrame(data), headers="keys", tablefmt="fancy_grid", showindex="never", numalign="center")
        await Wallets_CLI.update_wallet_balance(config_data['LAST_WALLET_SELECTED'], sol_balance)
        loading_spinner.stop()
        print(dataframe)
        print()
        return
...
class Main_CLI():
    ...
    async def first_login():
        ...
        f.display_logo()
        print("Welcome to Jupiter Sniper. Made with \u2764\ufe0f  by Krakovia\n")
        ...
        while True:
            ...
            if rpc_url == "":
                print(f"{c.RED}This is your first login, please enter a Solana RPC URL Endpoint. (If unsure, use https://api.mainnet-beta.solana.com).{c.RESET}")
            else:
                break
        await Config_CLI.prompt_discord_webhook()
        await Config_CLI.prompt_telegram_api()
    ...
    async def main_menu():
        """Main menu for CLI."""
        f.display_logo()
        print("Welcome to the Jupiter Python Sniper! Made with \u2764\ufe0f  by Krakovia\n")
        ...
        match cli_prompt_main_menu:
            ...
            case "About":
                ...
                print("DESCRIPTION")
                description = (
                    "This tool is a commande-line interface to use Jupiter Exchange faster Made with \u2764\ufe0f  by Krakovia" + 
